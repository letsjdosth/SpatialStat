...
class MCMC_base:
    ...
    def generate_samples(self, num_samples, pid=None, verbose=True, print_iter_cycle=500, first_time_est=100):
        start_time = time.time()
        for i in range(1, num_samples):
            self.sampler(iter_idx=i)
            
            if i==first_time_est and verbose:
                elap_time_head_iter = time.time()-start_time
                estimated_time = (num_samples/first_time_est)*elap_time_head_iter
